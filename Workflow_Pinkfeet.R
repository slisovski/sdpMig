...
parms <- list(
  MaxT   = 70,  ## Maximum time
  MaxX   = 100,  ## Maximum body condition
  NSites = 3,    ## Excl. Breeding site
  ...
  ## Terminal Reward
  xFTReward = c(0, 59, 60, 66, 67, 70),
  yFTReward = c(0, 0,   2,  2,  0,  0),
  
  
  ### Site specific Parameters ###
  path = "g://Dropbox//Modelling_Migration_SDP//Packages//sdpMig//Parameters//Pinkfeet.csv",
...
Out.file <- "test_pinkfeet.csv"
write("Scenario,mort,stag1,stag2,stag3,meanX1,meanX2,meanX3,fuelrate1,fuelrate2,fuelrate3",
...
for (scenario in 1:length(sims)) {
  ...
  sdp  <- makeSDPmig(parms, "Pinkfooted Geese")
